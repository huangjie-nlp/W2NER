...
class W2NER(nn.Module):
    ...
    def forward(self, data):
        ...
        :return:
        '''
        bert_inputs = data['input_ids'].to(self.device)
        attention_mask = data["attention_mask"].to(self.device)
        grid_mask2d = data['mask2d'].to(self.device)
        dist_inputs = data['dist_input'].to(self.device)
        pieces2word = data['pieces2word'].to(self.device)
        sent_length = data['length'].to(self.device)

        # bert_embs = self.bert(input_ids=bert_inputs, attention_mask=bert_inputs.ne(0).float())
        bert_embs = self.bert(input_ids=bert_inputs, attention_mask=attention_mask)
